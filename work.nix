...
let
  ...
  matterhornNotify = pkgs.writeShellScript "notify" ''
    ...
    /home/jared/src/personal/notify-send.sh/notify-send.sh -u "$urgency" -f -t 60000 -i ${inputs.matterhorn}/logo/matterhorn-icon.svg -a Matterhorn -c "im.received" -- "Matterhorn message from $2" "$m"
  '';
...
in {
  ...
  age.secrets."matterhorn-config.ini".owner = "jared";
  home-manager.users.jared = let cfg = config;
  in { config, ... }: {
    ...
    programs.git = {
      userName = pkgs.lib.mkForce "Jared Weakly";
      userEmail = pkgs.lib.mkForce "jweakly@galois.com";
      includes = [{
        condition = "gitdir:~/src/personal/";
        contents = {
          user = {
            name = "jared-w";
            email = "jaredweakly@gmail.com";
          };
        };
      }];
    };
